...
import com.zh.raback.service.ClientService;
...
/**
 * REST controller for managing {@link com.zh.raback.domain.Client}.
 */
@RestController
@RequestMapping("/api")
public class ClientResource {
     ...
     */
    @GetMapping("/clients")
    public ResponseEntity<List<ClientDTO>> getAllClients(@RequestParam(value = "search",required = false) String search,Pageable pageable) {
        log.debug("REST request to get a page of Clients");

        if (StringUtils.isNotBlank(search)){

            Node rootNode = new RSQLParser().parse(search);
            Specification<Client> specification = rootNode.accept(new CustomRsqlVisitor<Client>());
            Page<ClientDTO> page = clientService.findAllBySearch(specification, pageable);
    ...
    public ResponseEntity<Void> deleteClients(@RequestParam(value = "id") List<Long> ids) {
